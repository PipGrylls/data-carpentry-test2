...
> > ## Solution {: .solution}
> > 
> > {: .solution} ggplot(data = surveys\_complete, {: .solution} mapping
> > = aes(x = species\_id, y = weight)) + {: .solution}
> > geom\_point(aes(color = plot\_type)) {: .solution}
...
> > ## Solution {: .solution}
> > 
> > {: .solution} yearly\_weight \<- surveys\_complete %\>% {:
> > .solution} group\_by(year, species\_id) %\>% {: .solution}
> > summarize(avg\_weight = mean(weight)) {: .solution} ggplot(data =
> > yearly\_weight, mapping = aes(x=year, y=avg\_weight)) + {:
> > .solution} geom\_line() + {: .solution}
> > facet\_wrap(vars(species\_id)) + {: .solution} theme\_bw() {:
> > .solution} \`\`\` {: .solution}
...
Page built on: 📆 2022-01-11 ‒ 🕢 14:00:56
