...
> > ## Solution {: .solution}
> > 
> > {: .solution} levels(sex)\[2:3\] \<- c(“female”, “male”) {:
> > .solution} sex \<- factor(sex, levels = c(“female”, “male”,
> > “undetermined”)) {: .solution} plot(sex) {: .solution}
...
> > ## Solution {: .solution}
> > 
> > {: .solution} surveys\(hindfoot_length[surveys\)hindfoot\_length \<=
> > 10\] \<- “0-10” {: .solution}
> > surveys\(hindfoot_length[surveys\)hindfoot\_length \> 10 &
> > surveys\(hindfoot_length <= 20] <- "11-20" {: .solution} surveys\)hindfoot\_length\[surveys\(hindfoot_length > 20 & surveys\)hindfoot\_length
> > \<= 30\] \<- “21-30” {: .solution}
> > surveys\(hindfoot_length[surveys\)hindfoot\_length \> 30 &
> > surveys\(hindfoot_length <= 40] <- "31-40" {: .solution} surveys\)hindfoot\_length\[surveys\(hindfoot_length > 40 & surveys\)hindfoot\_length
> > \<= 50\] \<- “41-50” {: .solution}
> > surveys\(hindfoot_length[surveys\)hindfoot\_length \> 50 &
> > surveys\(hindfoot_length <= 60] <- "51-60" {: .solution} surveys\)hindfoot\_length\[surveys$hindfoot\_length
> > \> 60\] \<- “61+” {: .solution}
> > surveys\(hindfoot_length <- as.factor(surveys\)hindfoot\_length) {:
> > .solution} plot(surveys$hindfoot\_length) {: .solution}
...
> > ## Solution {: .solution}
> > 
> > {: .solution} surveys %\>% {: .solution} filter(year \< 1995) %\>%
> > {: .solution} select(year, sex, weight) {: .solution}
...
> > ## Solution {: .solution}
> > 
> > {: .solution} surveys\_final \<- surveys %\>% {: .solution}
> > filter(year \< 1995, year \!= 2001, plot\_type == ‘Control’) %\>% {:
> > .solution} select(record\_id, month, year, sex, weight) {:
> > .solution}
...
> > ## Solution {: .solution}
> > 
> > {: .solution} surveys\_hindfoot\_cm \<- surveys %\>% {: .solution}
> > filter(\!is.na(hindfoot\_length)) %\>% {: .solution}
> > mutate(hindfoot\_cm = hindfoot\_length / 10) %\>% {: .solution}
> > filter(hindfoot\_cm \< 3) %\>% {: .solution} select(species\_id,
> > hindfoot\_cm) {: .solution}
...
> > ## Solution {: .solution}
> > 
> > {: .solution} surveys\_simplified \<- surveys %\>% {: .solution}
> > mutate(weight\_simplified = {: .solution} ifelse(weight \<=
> > mean(weight, na.rm = TRUE), 1, 2)) {: .solution}
> 
...
> > ## Solution {: .solution}
> > 
> > {: .solution} surveys %\>% {: .solution} count(plot\_type) {:
> > .solution}
> 
...
> > ## Solution {: .solution}
> > 
> > {: .solution} surveys %\>% {: .solution}
> > filter(\!is.na(hindfoot\_length)) %\>% {: .solution}
> > group\_by(species\_id) %\>% {: .solution} summarize( {: .solution}
> > mean\_hindfoot\_length = mean(hindfoot\_length), {: .solution}
> > min\_hindfoot\_length = min(hindfoot\_length), {: .solution}
> > max\_hindfoot\_length = max(hindfoot\_length), {: .solution} n = n()
> > {: .solution} ) {: .solution}
> 
...
> > ## Solution {: .solution}
> > 
> > {: .solution} surveys %\>% {: .solution} filter(\!is.na(weight))
> > %\>% {: .solution} group\_by(year) %\>% {: .solution} filter(weight
> > == max(weight)) %\>% {: .solution} select(year, genus, species,
> > weight) %\>% {: .solution} arrange(year) {: .solution}
...
> > ## Solution {: .solution}
> > 
> > {: .solution} surveys\_wide\_genera \<- surveys %\>% {: .solution}
> > group\_by(plot\_id, year) %\>% {: .solution} summarize(n\_genera =
> > n\_distinct(genus))%\>% {: .solution} pivot\_wider(names\_from =
> > year, values\_from = n\_genera) {: .solution}
> > head(surveys\_wide\_genera) {: .solution}
> 
...
> > ## Solution {: .solution}
> > 
> > {: .solution} surveys\_wide\_genera %\>% {: .solution}
...
> > ## Solution {: .solution}
> > 
> > {: .solution} surveys\_long \<- surveys %\>% {: .solution}
> > pivot\_longer(cols = c(hindfoot\_length, weight), names\_to =
> > “measurement”, values\_to = “value”) {: .solution}
> 
...
> > ## Solution {: .solution}
> > 
> > {: .solution} surveys\_long %\>% {: .solution} group\_by(year,
> > measurement, plot\_type) %\>% {: .solution} summarize(mean\_value =
> > mean(value, na.rm=TRUE)) %\>% {: .solution} pivot\_wider(names\_from
> > = measurement, values\_from = mean\_value) {: .solution}
...
Page built on: 📆 2022-01-11 ‒ 🕢 14:00:58
