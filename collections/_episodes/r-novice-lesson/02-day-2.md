...
> > 
> {: .solution}
>  surveys\(hindfoot_length[surveys\)hindfoot\_length \<=
> > 10\] \<- “0-10” 
> {: .solution}
> 
> > surveys\(hindfoot_length[surveys\)hindfoot\_length \> 10 &
> > surveys\(hindfoot_length <= 20] <- "11-20" 
> {: .solution}
>  surveys\)hindfoot\_length\[surveys\(hindfoot_length > 20 & surveys\)hindfoot\_length
> > \<= 30\] \<- “21-30” 
> {: .solution}
> 
> > surveys\(hindfoot_length[surveys\)hindfoot\_length \> 30 &
> > surveys\(hindfoot_length <= 40] <- "31-40" 
> {: .solution}
>  surveys\)hindfoot\_length\[surveys\(hindfoot_length > 40 & surveys\)hindfoot\_length
> > \<= 50\] \<- “41-50” 
> {: .solution}
> 
> > surveys\(hindfoot_length[surveys\)hindfoot\_length \> 50 &
> > surveys\(hindfoot_length <= 60] <- "51-60" 
> {: .solution}
...
> > .solution} plot(surveys$hindfoot\_length) 
> {: .solution}
>  {:
> > .solution} {.challenge}
...
> > 
> {: .solution}
>  surveys %\>% {: .solution} filter(year \< 1995) %\>%
> > 
> {: .solution}
>  select(year, sex, weight) {: .solution} {: .solution}
> > {.challenge}
...
> > 
> {: .solution}
>  surveys\_final \<- surveys %\>% {: .solution}
...
> > 
> {: .solution}
>  surveys\_hindfoot\_cm \<- surveys %\>% {: .solution}
...
> > 
> {: .solution}
>  surveys\_simplified \<- surveys %\>% {: .solution}
> > mutate(weight\_simplified = 
> {: .solution}
>  ifelse(weight \<=
> > mean(weight, na.rm = TRUE), 1, 2)) 
> {: .solution}
>  {: .solution}
> 
...
> > 
> {: .solution}
>  surveys %\>% {: .solution} count(plot\_type) {:
> > .solution} 
> {: .solution}
> 
> 
...
> > 
> {: .solution}
>  surveys %\>% {: .solution} filter(\!is.na(weight))
> > %\>% 
> {: .solution}
>  group\_by(year) %\>% {: .solution} filter(weight
> > == max(weight)) %\>% 
> {: .solution}
>  select(year, genus, species,
> > weight) %\>% 
> {: .solution}
>  arrange(year) {: .solution} {: .solution}
> > {.challenge}
...
> > 
> {: .solution}
>  surveys\_wide\_genera \<- surveys %\>% {: .solution}
> > group\_by(plot\_id, year) %\>% 
> {: .solution}
>  summarize(n\_genera =
> > n\_distinct(genus))%\>% 
> {: .solution}
>  pivot\_wider(names\_from =
> > year, values\_from = n\_genera) 
> {: .solution}
> 
...
> > 
> {: .solution}
>  surveys\_wide\_genera %\>% {: .solution}
...
> > 
> {: .solution}
>  surveys\_long \<- surveys %\>% {: .solution}
...
> > 
> {: .solution}
>  surveys\_long %\>% {: .solution} group\_by(year,
> > measurement, plot\_type) %\>% 
> {: .solution}
>  summarize(mean\_value =
> > mean(value, na.rm=TRUE)) %\>% 
> {: .solution}
>  pivot\_wider(names\_from
> > = measurement, values\_from = mean\_value) 
> {: .solution}
>  {:
> > .solution} {.challenge}
...
Page built on: 📆 2022-01-14 ‒ 🕢 12:20:19
