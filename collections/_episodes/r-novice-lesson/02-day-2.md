...
> 
> > > ## Solution\> \>\> surveys\(hindfoot_length[surveys\)hindfoot\_length \<= 10\] \<- “0-10”
> > > 
...
> > plot(surveys$hindfoot\_length)\> {: .solution}
...
> > > ## Solution\> \>\> surveys %\>%
> > > 
> > > filter(year \< 1995) %\>% select(year, sex, weight)\> {:
> > > .solution}
...
> 
> > > ## Solution\> \>\> surveys\_final \<- surveys %\>%
> > > 
> > > filter(year \< 1995, year \!= 2001, plot\_type == ‘Control’) %\>%
> > > select(record\_id, month, year, sex, weight)\> {: .solution}
...
> 
> > > ## Solution\> \>\> surveys\_simplified \<- surveys %\>%
> > > 
> > > mutate(weight\_simplified = ifelse(weight \<= mean(weight, na.rm =
> > > TRUE), 1, 2))\> {: .solution}
> 
...
> > > ## Solution\> \>\> surveys %\>%
> > > 
> > > filter(\!is.na(hindfoot\_length)) %\>% group\_by(species\_id) %\>%
> > > summarize( mean\_hindfoot\_length = mean(hindfoot\_length),
> > > min\_hindfoot\_length = min(hindfoot\_length),
> > > max\_hindfoot\_length = max(hindfoot\_length), n = n() )\> {:
> > > .solution}
> 
...
> > > ## Solution\> \>\> surveys %\>%
> > > 
> > > filter(\!is.na(weight)) %\>% group\_by(year) %\>% filter(weight ==
> > > max(weight)) %\>% select(year, genus, species, weight) %\>%
> > > arrange(year)\> {: .solution}
...
>     `?n_distinct` for more.
> 
> > > ## Solution\> \>\> surveys\_wide\_genera \<- surveys %\>%
> > > 
> > > group\_by(plot\_id, year) %\>% summarize(n\_genera =
> > > n\_distinct(genus))%\>% pivot\_wider(names\_from = year,
> > > values\_from = n\_genera)
> > 
...
> 
> > > ## Solution\> \>\> surveys\_wide\_genera %\>%
> > > 
...
> 
> > > ## Solution\> \>\> surveys\_long \<- surveys %\>%
> > > 
...
Page built on: 📆 2022-01-11 ‒ 🕢 09:46:31
